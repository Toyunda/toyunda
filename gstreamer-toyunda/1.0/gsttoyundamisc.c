...
static GstBuffer*	gst_toyunda_get_image_data(GstToyunda *toyunda, gchar *image_file, uint *width, uint *height)
{
	...
	*height = y;
	...
	img->buffer = gst_buffer_new();
	mem = gst_allocator_alloc(NULL, x * y * 4, NULL);
	gst_memory_map(mem, &memmap, GST_MAP_WRITE);
	memcpy(memmap.data, data, x * y * 4);
	g_free(data);
	gst_memory_unmap(mem, &memmap);
	gst_buffer_insert_memory(img->buffer, -1, mem);
	
	gst_buffer_add_video_meta(img->buffer, GST_VIDEO_FRAME_FLAG_NONE, GST_VIDEO_FORMAT_BGRA, x, y);
