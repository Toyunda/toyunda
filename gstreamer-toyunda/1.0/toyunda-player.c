...
void	on_added_pad(GstElement *elem, GstPad	*pad, gboolean b, gpointer *data)
{
	GstCaps*	caps;
	GstStructure*	str;
	GstStructure*	str2;
...
void	gstreamer_create_pipeline()
{
	...
	GstElement	*convert, *resample, *videoconvert, *videoscale;
	GstBus*		bus;
	GstPad*		gpad, *gpad2;
	guint		watch_id;
	...
	gst_bus_set_sync_handler (bus, (GstBusSyncHandler) xoverlay_cb, pipeline, NULL);
	...
	gst_bin_add_many(videobin, queuev, capsfilter, toyunda, videoconvert, videosink, videoscale, NULL);
	gst_element_link_many(queuev, videoconvert, videosink, NULL);
